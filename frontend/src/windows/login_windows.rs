            ...
            ui.horizontal(|ui| {
                ...
                ui.vertical_centered(|ui| {
                    ...
                    ui.horizontal(|ui| {
                        ...
                        if ui
                            .link(egui::RichText::new("密码登录").size(18.0))
                            .clicked()
                        {
                        ...
                        ui.add(egui::Label::new(egui::RichText::new("|").size(18.0)));
                        if ui
                            .link(egui::RichText::new("短信登录").size(18.0))
                            .clicked()
                        {
                    ...
                    ui.vertical_centered(|ui| match login_method.as_str() {
                        "扫码登录" => {
                            ui.add_space(20.0);
...
fn ui_qrcode_login(ui: &mut egui::Ui, app: &mut Myapp) {
    ...
    if response.clicked() {
        ...
        if let Some(task_id) = &app.qrcode_polling_task_id {
            let _ = app.task_manager.cancel_task(task_id);
    ...
    if should_refresh {
        match common::login::qrcode_login(&app.client) {
            Ok(code) => {
                let login_string = format!(
                    "https://account.bilibili.com/h5/account-h5/auth/scan-web?navhide=1&callback=close&qrcode_key={}&from=main-fe-header",
                    code
                );
...
fn ui_password_login(ui: &mut egui::Ui, app: &mut Myapp) {
    ui.vertical_centered(|ui| {
        ...
        common_input(
            ui,
            "密码",
            &mut app.login_input.password,
            "请输入密码",
            true,
        );
        ui.add_space(20.0);
        let button = egui::Button::new(
            egui::RichText::new("登录")
                .size(15.0)
                .color(egui::Color32::WHITE),
        )
...
pub fn phone_input(
    ...
    if ui
        .link(egui::RichText::new("发送短信").size(12.0))
        .clicked()
    {
...
pub fn create_qrcode(ctx: &egui::Context, content: &str) -> Option<TextureHandle> {
    ...
    Some(ctx.load_texture("qrcode", color_image, egui::TextureOptions::default()))
}

// Base64图像加载函数
fn load_image_from_base64(
    ctx: &egui::Context,
    name: &str,
    base64_data: &str,

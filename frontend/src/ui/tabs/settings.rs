...
pub fn render(app: &mut Myapp, ui: &mut egui::Ui) {
    ui.horizontal(|ui| {
        ...
        if response.clicked() {
            match save_config(
                &mut app.config,
                Some(&app.push_config),
                Some(&app.custom_config),
                None,
            ) {
                Ok(_) => {
                    log::info!("设置保存成功");
...
pub fn globle_setting(app: &mut Myapp, ui: &mut egui::Ui) {
    ui.horizontal(|ui| {
        common_input(
            ui,
            "请输入账号1预填手机号: ",
            &mut app.custom_config.preinput_phone1,
            "请输入账号1绑定的手机号",
            true,
        );
        common_input(
        ...
        match app.custom_config.captcha_mode {
            1 => {
                ...
                common_input(
                    ui,
                    "请输入ttocr key: ",
                    &mut app.custom_config.ttocr_key,
                    "请输入ttocr key",
                    true,
                );
            }
    ...
    ui.horizontal(|ui| {
        ...
        common_input(
            ui,
            "",
            &mut app.custom_config.custom_ua,
            "请输入自定义UA",
            false,
        );
    });
...
fn custom_selection_control(ui: &mut egui::Ui, selected: &mut usize, options: &[&str]) -> bool {
    ...
    ui.horizontal(|ui| {
        for (idx, option) in options.iter().enumerate() {
            ...
            let button = egui::Button::new(egui::RichText::new(*option).size(15.0).color(
                if is_selected {
                    egui::Color32::WHITE
                } else {
                    egui::Color32::BLACK
    ...
    if app.push_config.enabled {
        ...
        ui.horizontal(|ui| {
            common_input(
                ui,
                "bark推送: ",
                &mut app.push_config.bark_token,
                "请输入推送地址, 只填token",
                true,
            );
            dynamic_caculate_space(ui, 180.0);
            common_input(
                ui,
                "pushplus推送: ",
                &mut app.push_config.pushplus_token,
                "请输入推送地址, 只填token",
                true,
            );
        });
        ...
        ui.horizontal(|ui| {
            common_input(
                ui,
                "方糖推送: ",
                &mut app.push_config.fangtang_token,
                "请输入推送地址: SCTxxxxxxx",
                true,
            );
            dynamic_caculate_space(ui, 180.0);
            common_input(
                ui,
                "钉钉机器人推送: ",
                &mut app.push_config.dingtalk_token,
                "请输入钉钉机器人token, 只填token",
                true,
            );
        });
        ...
        ui.horizontal(|ui| {
            common_input(
                ui,
                "企业微信推送: ",
                &mut app.push_config.wechat_token,
                "请输入企业微信机器人token",
                true,
            );
            dynamic_caculate_space(ui, 180.0);
        ...
        ui.horizontal(|ui| {
            common_input(
                ui,
                "smtp服务器地址: ",
                &mut app.push_config.smtp_config.smtp_server,
                "请输入smtp服务器地址",
                true,
            );
            dynamic_caculate_space(ui, 180.0);
            common_input(
                ui,
                "smtp服务器端口: ",
                &mut app.push_config.smtp_config.smtp_port,
                "请输入smtp服务器端口",
                true,
            );
        });
        ui.horizontal(|ui| {
            common_input(
                ui,
                "邮箱账号: ",
                &mut app.push_config.smtp_config.smtp_from,
                "请输入发件人邮箱",
                true,
            );
            dynamic_caculate_space(ui, 180.0);
            common_input(
                ui,
                "授权密码: ",
                &mut app.push_config.smtp_config.smtp_password,
                "请输入授权密码",
                true,
            );
            dynamic_caculate_space(ui, 180.0);
        ...
        ui.horizontal(|ui| {
            ...
            common_input(
                ui,
                "收件人邮箱: ",
                &mut app.push_config.smtp_config.smtp_to,
                "请输入收件人邮箱",
                true,
            );
        });
        ui.horizontal(|ui| {
            common_input(
                ui,
                "gotify地址: ",
                &mut app.push_config.gotify_config.gotify_url,
                "请输入gotify服务器地址, 只填写地址",
                false,
            );
            dynamic_caculate_space(ui, 180.0);
            common_input(
                ui,
                "gotify的token",
                &mut app.push_config.gotify_config.gotify_token,
                "请输入gotify的token",
                true,
            )
        });
